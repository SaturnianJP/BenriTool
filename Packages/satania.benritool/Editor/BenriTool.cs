...
namespace satania.benritool
{
    public class BenriTool : EditorWindow
    {
        ...
        public GameObject[] _prefabs = new GameObject[0];
        public GameObject[] _ref_prefabs = new GameObject[0];

        private string savePath = "Assets/さたにあ/BenriTool/Generate/";
        #endregion
        ...
        private void SetMaterials(GameObject from, GameObject to)
        {
            if (to == null)
                return;

            Dictionary<string, Material[]> _meshRenderers = new Dictionary<string, Material[]>();
            Dictionary<string, Material[]> _skinRenderers = new Dictionary<string, Material[]>();

            foreach (var mesh in from.GetComponentsInChildren<MeshRenderer>())
            {
                string meshpath = GetHierarchyPath(mesh.gameObject);
                _meshRenderers.Add(meshpath, mesh.sharedMaterials);
            }

            foreach (var skin in from.GetComponentsInChildren<SkinnedMeshRenderer>())
            {
        ...
        private void drawArray(string name, string displayname)
        {
            ...
            EditorGUILayout.PropertyField(stringsProperty, new GUIContent(displayname), true);
        ...
        private void ShowGUI()
        {
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (GUILayout.Button("コピー開始"))
                {
                    ...
                    for (int i = 0; i < _ref_prefabs.Length; i++)
                    {
                        foreach (var prefab in _prefabs)
                        {
                            ...
                            SetMaterials(_ref_prefabs[i], _pref);

                            //ファイル名に使用できない文字を取得
                            char[] invalidChars = System.IO.Path.GetInvalidFileNameChars();

                            string filename = _pref.name + _ref_prefabs[i].name;
                            if (filename.Length > 50)
                                filename = filename.Substring(0, 50);

                            for (int j = 0; j < invalidChars.Length; j++)
                            {
                                filename = filename.Replace(invalidChars[j].ToString(), "");
                            }
